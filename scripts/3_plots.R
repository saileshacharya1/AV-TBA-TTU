...
activities <-
  c(
    tba_1 = "Listening to music, radio, or other audio",
    tba_2 = "Singing, dancing",
    tba_3 = "Interacting with other passengers",
    tba_4 = "Talking on phone",
    tba_5 = "Texting, emailing, or teleconference",
    tba_6 = "Reading newspapers, books, websites, etc.",
    tba_7 = "Using social websites or apps",
    tba_8 = "Watching movies/TV/other entertainment",
    tba_9 = "Playing games",
    tba_10 = "Working or studying",
    tba_11 = "Caring for or playing with children or pets",
    tba_12 = "Eating food, drinking beverage, smoking",
    tba_13 = "Sleeping or snoozing",
    tba_14 = "Viewing scenery; watching people",
    tba_15 = "Thinking or daydreaming",
    tba_16 = "Watching the road",
    tba_17 = "Other"
  )
df_p2$activity <- as.factor(as.character(activities[df_p2$activity]))
rm(activities)

# reorder the levels of activites
...
p2 <- df_p2 %>% ggplot(aes(x = percent, y = activity, fill = variable)) +
  ...
  geom_text(aes(label = count),
    hjust = ifelse(df_p2$x_max == 0,
      ifelse(df_p2$count > 70, -0.5, 1.5),
      ifelse(df_p2$count > 70, 1.5, -0.5)
    ),
...
ggsave(
  ...
  plot = p2, width = 6.5, height = 6.5, unit = "in", dpi = 1000
)
rm(p2, df_p2)
dev.off()
#------------------------------------------------------------------------------#
